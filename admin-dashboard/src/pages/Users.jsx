...
const UsersPage = () => {
  ...
  // Toggle visibility of vaccine details
              ...
              {users.map((user) => (
                <tr key={user._id} className="border-b border-gray-300">
                  ...
                  <td className="px-4 py-2">
                    ...
                    >
                      {expandedUserId === user._id ? "Hide" : "Show"} Vaccine Details
                    </button>
                        ...
                        {user.vaccinesTaken.length > 0 ? (
                          user.vaccinesTaken.map((vaccine) => (
                            <div key={vaccine._id}>
                              ...
                              <p><strong>Vaccination Date:</strong> {new Date(vaccine.vaccinationDate).toLocaleDateString()}</p>
                              <p><strong>Batch Number:</strong> {vaccine.batchNumber}</p>
                              <p><strong>Health Post:</strong> {vaccine.healthPostName}</p>
                              <p><strong>District:</strong> {vaccine.district}</p>
                              <p><strong>Province:</strong> {vaccine.province}</p>
                              <p><strong>Local Body:</strong> {vaccine.localBody}</p>
                              <hr className="my-2" />
                            </div>
                          ))
                        ) : (
